...
export default {
    async play(message: Message, song: string) {
        ...
        let metadata: { title: any, url: any, thumbnail: any, embedTitle: string | undefined, embedDescription: string | undefined };
        ...
        if (YTVideoRegex.test(song) || YTPlaylistRegex.test(song) || YTMixRegex.test(song)) {
            if (YTPlaylistRegex.test(song) || YTMixRegex.test(song)) {
                try {
                    ...
                    metadata = await getYTMetadata(playlist, 'playlist'); 
                } catch {
                    // Could not find the playlist.
                    songQueue.push({type: 'youtube', url: song, requester: requester});
                    metadata = await getYTMetadata(song, 'video');
                }
            } else {
                songQueue.push({type: 'youtube', url: song, requester: requester});
                metadata = await getYTMetadata(song, 'video');
        ...
        // Check if a queue exists for that server.;
        ...
        } else if (serverQueue.songQueue.length < 1) {
...
async function getStream(currentSong: any) {
    let stream;
    const metadata = await getYTMetadata(currentSong.url, 'video');
...
async function playManager(guildId: string) {
    const serverQueue = serverQueues.get(guildId);
    ...
    // Generate stream.
    const {stream, metadata} = await getStream(currentSong);
    ...
    connection.on(VoiceConnectionStatus.Disconnected, async () => {
        try {
            ...
            serverQueue.textChannel.send({embeds: [disconnectedEmbed]});
        }
    })
...
async function getYTMetadata(request: any, type: string) {
    let title;
    let thumbnail;
    let url;
    let embedTitle;
